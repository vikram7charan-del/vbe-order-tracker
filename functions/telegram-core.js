...
function focusItemsOf(data, own){
  ...
  return items.map(f=>{
    const c=byId[f.id]; let done=false;
    if(c){ const tp=topics(c); if(tp[f.i]&&tp[f.i].done) done=true; }
    return {...f, cname:c?c.name:'', done};
  }).filter(f=>!f.done).sort((a,b)=>(a.until||0)-(b.until||0)); // सबसे लेट/जल्दी deadline पहले
}
...
function focusDash(data, own, now){
  ...
  active.forEach((f,i)=>{
    ...
    out+=`\n${i+1}. ${f.t}${f.cname?' ('+f.cname+')':''}\n   ⏱ ${run} से चालू${late?' · 🔴 '+fmtDur(now-f.until)+' लेट':' · ⏳ '+fmtDur(f.until-now)+' बाकी'}\n`;
  });
  if(queue.length) out+=`\n📋 Queue में और ${queue.length} काम — ऊपर वाले निपटते ही अगले।`;
...
function answer(data,textRaw,now){
  ...
  if(has('focus','फोकस','फ़ोकस','फॉक्स')){
    ...
    // 🎯 Start Focus — काम चुनकर focus में डालो (एक बार में 5 तक)
    if(/शुरू|start|चुन|जोड़|\badd\b|डाल/.test(t)){
      const pend=allTasks(data.contacts).filter(x=>!x.done);
      const rank=pend.map(x=>({x,s:(x.pri==='high'?30:0)+(x.at&&new Date(x.at).getTime()<now?40:0)+(x.at?10:0)}))
        .sort((a,b)=>b.s-a.s).slice(0,8).map(o=>o.x);
      if(!rank.length) return {text:'✓ कोई बाकी काम नहीं — focus में डालने को कुछ नहीं।'};
      const whoLbl=OWNERS[own||'v'];
      let out=`🎯 *Focus शुरू करें* — ${whoLbl}\nकिन कामों पर focus? (एक बार में ${FOCUS_ACTIVE} तक) — नीचे 🎯 दबाएँ:\n\n`;
      rank.forEach((x,i)=>{ out+=`${i+1}. ${x.pri==='high'?'🔴 ':''}${CATS[x.cat]?CATS[x.cat].logo+' ':''}${x.t} (${x.name})\n`; });
      const btns=rank.map((x,i)=>({text:'🎯 '+(i+1),callback_data:('f|'+(own||'v')+'|'+x.cid+'|'+x.ti).slice(0,64)}));
      const rows=[]; for(let i=0;i<btns.length;i+=4) rows.push(btns.slice(i,i+4));
      rows.push([{text:'📋 '+OWN_SHORT[own||'v']+' का focus देखो',callback_data:'fd|'+(own||'v')}]);
      return {text:out, reply_markup:{inline_keyboard:rows}};
    }
