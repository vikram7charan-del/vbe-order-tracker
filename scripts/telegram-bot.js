...
🕘 समय के साथ: "कल सुबह 9 बजे मनोज को तार लाने भेजो"
   → काम बन जाए + समय पर reminder खुद आए
✅ किसी list के नीचे ✅1 ✅2 बटन दबाओ = काम पूरा

⌨️ Commands: /today /kal /late /pending /zaroori
   /report /call /market /computer /search DVR /help`;

/* ⌨️ Slash commands → आम भाषा में बदलो */
...
/* 🕘 हिंदी समय-समझ: "कल सुबह 9 बजे" → {at, बचा हुआ text}। कुछ न मिले तो null।
   token-आधारित — Devanagari पर \b भरोसेमंद नहीं, इसलिए शब्द-दर-शब्द। */
function parseWhen(textRaw, now){
  ...
  for(let i=0;i<toks.length;i++){
    ...
    if(wl==='बजे'){ has=true; continue; } // घंटा पिछले token में आ चुका
...
function answer(data,textRaw,now){
  ...
  /* ⌨️ Slash commands → आम भाषा में बदलकर दोबारा route */
  if(text[0]==='/'){
  ...
  /* ➕ काम जोड़ना — "X को ..." (क्रिया के साथ), साथ में समय भी समझो */
  const when=parseWhen(text,now);            // "कल सुबह 9 बजे" कहीं भी हो
  const forMatch=when?when.text:text;        // समय हटाकर नाम+काम ढूँढो
  ...
  if(addM && /(बोल|बता|कह|मंगवा|मँगवा|खरीद|भेज|लाना|लाने|देना|दिलवा|करवा|बनवा|पूछ|याद|दे दो|दो|करना|कर दे)/.test(addM[2])){
    ...
    if(p){
      ...
      const whenLbl=when?`\n🕘 समय: ${istParts(when.at)} ${new Intl.DateTimeFormat('en-GB',{timeZone:'Asia/Kolkata',hour:'2-digit',minute:'2-digit',hour12:false}).format(new Date(when.at))} — reminder लगेगा`:'';
    ...
    for(const u of ups){
      ...
      if(ans&&ans.addTask){
        try{
          ...
          if(dc.exists){
            const c=dc.data(); const tps=topics(c);
            const nt={t:ans.addTask.text,done:false,doneAt:null,addedAt:new Date().toISOString(),src:'tg'};
            if(ans.addTask.at) nt.at=ans.addTask.at;
            tps.push(nt);
            const hist=Array.isArray(c.hist)?c.hist:[];
...
module.exports={answer,collectAll,phonKey,findByName,personIn,catIn,istParts,allTasks,parseWhen};
